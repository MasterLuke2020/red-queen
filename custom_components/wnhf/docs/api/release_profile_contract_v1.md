...
`wnhf.execute` is retained as a legacy compatibility path only. Qualification uses the
persistent per-installation Stage-4.7 execution evidence store and is diagnostic only.
