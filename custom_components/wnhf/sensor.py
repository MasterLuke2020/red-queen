...
async def _async_setup_entities(
    ...
    async_add_entities(
        [
            ...
            *(
                WNHFCoverPositionSensor(hass, engine, cover)
                for cover in sorted(
                    engine._require_house().enabled_covers,
...
class WNHFCoverPositionSensor(SensorEntity):
    """Read-only Home Assistant position sensor for one semantic cover."""
    ...
    def __init__(
        ...
        self._attr_name = f"Red Queen {cover.name} – Position"
        self._attr_unique_id = f"wnhf_cover_position_{slug}"
        self._attr_suggested_object_id = f"wnhf_cover_position_{slug}"
        self._attr_device_info = room_device_info(engine, cover.room_id)

    async def async_added_to_hass(self) -> None:
        """Refresh whenever objective cover feedback changes."""
    ...
    def available(self) -> bool:
        """Expose availability only while the configured position source is usable."""
        entity_id = self.cover_object.closed_percent_feedback_entity_id
        if not self.cover_object.enabled or not entity_id:
            return False
        state = self.hass.states.get(entity_id)
        return (
            state is not None
            and state.state not in {"unknown", "unavailable"}
            and self.engine.cover_snapshot(self.cover_object.object_id).position_available
        )

    @property
    def native_value(self) -> int | None:
        """Return HA-standard position: 0 = closed, 100 = open."""
        return self.engine.cover_snapshot(self.cover_object.object_id).current_position

    @property
    def extra_state_attributes(self) -> dict[str, Any]:
        """Expose the raw PLC scale without claiming set-position support."""
        ...
        return {
            ...
            "source_entity_id": self.cover_object.closed_percent_feedback_entity_id,
            "closed_percent": snapshot.closed_percent,
            "position_scale": "0_closed_100_open",
            "source_scale": "0_open_100_closed",
            "read_only": True,
            "set_position_supported": False,
